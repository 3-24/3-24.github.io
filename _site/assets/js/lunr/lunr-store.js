var store = [{
        "title": "이분그래프",
        "excerpt":"1. 정의 그래프 $G = (V,E)$에 대해 다음 조건을 만족하면 이분그래프bipartite graph 라고 합니다. 공집합이 아닌 $V$의 분할partition ${V_1 ,V_2}$가 있어서 이와 같은 이분그래프에 대해 추가적인 표기법으로 $G = (V_1, V_2, E)$를 사용하기도 합니다. Figure 1과 같은 형태입니다. Figure 1. 이분그래프의 예 2. 판별 따라서 어떤 그래프가 이분그래프인지 아닌지 판별할...","categories": ["Mathematics","Graph Theory"],
        "tags": ["이분그래프"],
        ...
        "excerpt":"주어진 실수 $a_{i,j}(1 \\le i \\le j \\le N)$에 대해 다음과 같이 정의된 $d_{i,j}$를 고려합니다. 이는 다음과 같은 $O(N^3)$짜리 동적계획법으로 해결할 수 있습니다. for (int i=1;i&lt;=N;i++){ D[i][i] = a[i][i]; } for (int l=1;i&lt;N;i++){ // l: length of interval for (int i=1;i&lt;=N-l;i++){ int j = i+l; D[i][j] = INT_MAX; for (int...","categories": ["Algorithm"],
        "tags": ["Knuth"],
