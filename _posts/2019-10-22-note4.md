...
* %rsp는 stack의 위치
* %rip는 프로그램 카운터, 즉 현재 실행되고 있는 코드의 위치를 가리키는 레지스터
...
condition codes에 기초하여 dest의 마지막 바이트를 0이나 1로 변경한다. 나머지는 건들지 않는다.
...
condition codes에 기초하여 program counter를 Dest로 변경시킨다.
...
오히려 while 문이 어셈블리 레벨에서는 더 복잡한데, 두 가지 방법이 존재한다.
